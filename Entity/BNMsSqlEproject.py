...
#Sqlserver数据库连接池
# @singleton
from Entity import db_host, db_pwd, db_user, db_eproject_dbname


class bnmssqleproject(object):
    __pool=None

    def __init__(self):
        self.conn = bnmssqleproject.getconn()
        self.cur = self.conn.cursor(as_dict=True)
        print('bnmssqleproject.conn===> ' + str(self.conn))

    @classmethod
    def getconn(cls):

        if bnmssqleproject.__pool is None:
            __pool=PooledDB(creator=pymssql, mincached=1, maxcached=5,
                                    host=db_host, user=db_user, password=db_pwd, database=db_eproject_dbname,charset='utf8')
